...
class AssetPipelineFilter implements Filter {
            ...
            if (attributeCache) {
                if (attributeCache.exists()) {
                    ...
                    if (file?.exists() && responseBuilder.ifModifiedSinceHeader && hasNotChanged(responseBuilder
                            .ifModifiedSinceHeader, attributeCache.getLastModified(), 
                            file)) {
                ...
                if (file.exists()) {
                    ...
                    if (modifiedSinceHeader && hasNotChanged(ifModifiedSinceHeader, new Date(file
                            .lastModified()), file )) {
                        responseBuilder.statusCode = 304
                    }
                    responseBuilder.headers.each { header ->
    ...
    boolean hasNotChanged(String ifModifiedSince, Date date, File file) {
