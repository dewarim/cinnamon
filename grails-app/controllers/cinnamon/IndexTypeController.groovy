...
//        foo = IndexType.get(62)
    ...
    def edit() {
        ...
        if (!indexTypeInstance) {
            ...
            return [indexTypeInstance       : indexTypeInstance,
                    indexers                : Holders.config.indexers,
                    dataTypeValues          : DataType.values(),
                    valueAssistanceProviders: Holders.config.vaProviders,
            ]
        }
    }
    ...
    def update() {
        ...
        if (indexTypeInstance) {
            ...
            try {
                log.debug("looking for class: " + params.indexerClassName)
                Class<Indexer> indexer = (Class<Indexer>) Class.forName(params.indexerClassName, true, Thread.currentThread().contextClassLoader)
                log.debug("looking for class: " + params.vaProviderClassName)
                Class<ValueAssistanceProvider> vaProviderClass =
                        (Class<ValueAssistanceProvider>) Class.forName(params.vaProviderClassName, true, Thread.currentThread().contextClassLoader)
                indexTypeInstance.indexerClass = indexer
                indexTypeInstance.vaProviderClass = vaProviderClass
            } catch (ClassNotFoundException ex) {
                log.debug("ClassNotFound: ", ex);
                flash.error = message(code: 'class.not.found', args: [ex?.cause?.message])
                ...
                log.warn("Failed to load classes",e)
