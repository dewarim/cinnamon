...
<div class="content">
	...
	<g:form method="post">
			...
			<table>
				...
				<tr class="prop">
					...
					<td class="value ${hasErrors(bean: user, field: 'language', 'errors')}">
						<g:select name="language.id" from="${UiLanguage.list()}" optionKey="id" value="${user?.language?.id}" optionValue="isoCode"/>
					</td>
				</tr>
		...
		<div class="buttons">
			<span class="button"><g:actionSubmit action="update" class="save" value="${message(code:'update')}"/></span>
		</div>
	</g:form>
</div>
</body></html>
