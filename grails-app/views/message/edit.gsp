    ...
    <div class="dialog">
        <table>
            ...
            <tr class="prop">
                ...
                <td class="value ${hasErrors(bean: message, field: 'message', 'errors')}">
        ...
        <table>
            ...
            <g:each in="${languages}" var="language">
                <tr>
                    <td class="name">
                        <label for="translation_${language}">
                            <g:message code="lang.${language.isoCode}" default="${language.isoCode}"/>
                        </label>
                    </td>
                    ...
                    <td class="value">
                        ...
                        <g:formRemote name="${language}" url="[controller: 'message', action: 'updateTranslationAjax']"
                                      onLoading="document.getElementById('${lang_div_id}').style.display='block';"
                                      onComplete="getElementById('${lang_submit_id}').style.display = 'none';return true;"
                                      update="[success: lang_div_id, failure: 'error']">
                            <input type="hidden" name="language" value="${language.id}"/>
                            <input type="hidden" name="messageId" value="${messageId}"/>
                            <input type="text"
                                   onKeypress="document.getElementById('${lang_submit_id}').style.display = 'block';
                                   document.getElementById('${lang_div_id}').style.display = 'none';"
                                   name="translation" id="translation_${language}"
                                   value="${translations[language.isoCode]?.encodeAsHTML()}"/>
                            <input type="submit" style="display:none;" name="save" id="${lang_submit_id}"
                                   value="${message(code: 'update.translation')}"/>
                        </g:formRemote>
                    </td>
                </tr>
            </g:each>
